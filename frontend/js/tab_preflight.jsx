...
function analysisHumanGroups(s) {
  ...
  const fix = [
    ...
    row("confirmedFindings", TR("Ваша работа, но есть находки проверок"),
      TR("починит «Ремонт» с галочкой «чинить мои строки»"), h.confirmedFindings, "var(--c-warning)"),
...
function WorkSummary({ summary, store, toast, onReload }) {
  ...
  return React.createElement("div", { className: "section" },
    ...
    React.createElement("div", { className: "card card-pad", style: { display: "flex", flexDirection: "column" } },
      ...
      /* Субтитры: термины вне прогона (кнопка «Проверить термины» в
         редакторе). Сегменты лежат в «готово», но «начисто» их звать нельзя —
         без этой строки они выпали бы из списка. */
      (s.todo.termsOff || []).length > 0 && React.createElement(Row, { label: TR("Термины не проверялись"),
        n: s.todo.termsOff.length, ids: s.todo.termsOff,
        hint: TR("у субтитров — отдельной кнопкой «Проверить термины»") }),
        ...
        return React.createElement(React.Fragment, { key: st.step },
          ...
          showModels && (pkey && setMod
            ? React.createElement(Select, { className: "select tk-select", value: mm[pkey] || "",
                ...
                (cat && cat.models || []).map(m => React.createElement("option", { key: m.id, value: m.id, disabled: m.ready === false }, m.label + (m.ready === false ? TR(" — нет ключа") : ""))))
...
const ASK_TITLE = {
  ...
  confirmedFindings: TR("Это ваши строки, а я нашла в них ошибку"),
...
function CheckQuestions({ summary, store, toast, onReload }) {
  ...
  const HINT = {
    confirmedFindings: TR("вы писали или заверяли эти строки, а проверки нашли ошибки — починит «Доделать сама» с галочкой «чинить мои строки»"),
