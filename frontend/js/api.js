...
(function () {
  ...
  /* Упрощённый режим организации: ни сумм, ни выбора моделей на экране.
     ...
     первый кадр рисуется раньше первого ответа, и без кэша тестировщик
     успел бы увидеть ровно то, что обещано не показывать. Источник правды
     всё равно сервер: пришёл ответ — переписываем. */
  function setHideCost(r) {
    if (!r || typeof r.hideCost !== "boolean") return;
    window.HIDE_COST = r.hideCost;
    try { localStorage.setItem("mcat_hide_cost", r.hideCost ? "1" : "0"); } catch (e) {}
  }
  ...
  window.API = {
    ...
    hasToken: () => !!getToken(),
    me:            ()                       => call("GET",    "/auth/me").then(r => { setHideCost(r); return r; }),
    /* ── Профиль и команды ──────────────────────────────────────────
    ...
    /* Содержание вкладки «Обучение». Язык сервер берёт из сессии сам —
       второй копии выбора языка в браузере быть не должно. */
    tutorial:      ()                       => call("GET",    "/tutorial"),
    ...
    // читателей каталога: каждая вкладка спрашивает /api/models сама, и
    // разложи мы это по вкладкам — часть экранов осталась бы с суммами.
    models:        ()                       => call("GET",    "/models").then(r => { setHideCost(r); return r; }),
    ...
    brief:         (pid)                    => call("GET",    `/projects/${pid}/brief`),
    setBrief:      (pid, body)              => call("POST",   `/projects/${pid}/brief`, body || {}),
    buildBrief:    (pid)                    => call("POST",   `/projects/${pid}/brief/build`, {}),
