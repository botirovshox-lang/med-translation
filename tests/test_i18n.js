/* Словари интерфейса: полнота и честность — ПО КАЖДОМУ языку.
 *
 ...
 *
 * Языки НЕ перечислены здесь списком: они берутся у window.I18N.langs,
 * то есть из того же места, откуда их берёт экран. Список в тесте пришлось
 * бы править при каждом новом языке — и первый же забытый язык проверялся
 * бы НИКОГДА, оставаясь наполовину русским. Русский пропускается: ключ
 * и есть русская строка, словаря у него нет и быть не должно.
 *
 * Что сторожится (для каждого языка):
...
/* Русский словаря не имеет: TR(s) === s побитово, и это свойство, а не дыра. */
const LANGS = window.I18N.langs.map(l => l.code).filter(c => c !== "ru");
check(LANGS.length > 0, "языки с переводом объявлены: " + LANGS.join(", "));
...
/* И обратно: то, что сервер отвечает ЧАСТО и без вставок, обязано быть
   ...
   каждый, и увидеть их по-русски в переведённом интерфейсе значит показать,
...
for (const lang of LANGS) {
  console.log("\n########## " + lang + " ##########");
  window.I18N.setLang(lang, true);

  console.log("=== 1. Полнота ===");
  const missing = [...used.keys()].filter(k => TR(k) === k && /[А-Яа-яЁё]/.test(k));
  check(missing.length === 0, lang + ": все строки из кода есть в словаре" +
  ...
  const parts = fs.readdirSync("frontend/i18n")
    .filter(f => f.startsWith(lang + ".") && f.endsWith(".json") && f !== lang + ".server.json");
  const catalog = {};
  ...
  check(notTranslated.length === 0, lang + ": самые частые отказы сервера переведены" +
  ...
  check(dirty.length === 0, lang + ": в переводе сообщений сервера не осталось русских букв" +
  ...
  check(TRS(SRV_SAMPLES[1]).includes("$1.00") && TRS(SRV_SAMPLES[1]).includes("$2.00"),
        lang + ": а числа подстановка не трогает");
  ...
  check(edge.length === 0, lang + ": пробелы по краям сохранены" + (edge.length ? " — сбито " + edge.length + ":" : ""));
  edge.slice(0, 20).forEach(k => console.log("       ± " + JSON.stringify(k) + " → " + JSON.stringify(catalog[k])));
  check(nl.length === 0, lang + ": переносов строк столько же" + (nl.length ? " — сбито " + nl.length : ""));
  nl.slice(0, 10).forEach(k => console.log("       ± " + JSON.stringify(k)));
  check(nbsp.length === 0, lang + ": неразрывных пробелов столько же" + (nbsp.length ? " — сбито " + nbsp.length : ""));
  ...
  check(copied.length === 0, lang + ": русских строк, скопированных как «перевод», нет" +
...
/* Обёртка сломала это один раз и сломает снова: шесть кодов причин лежат
   ...
   с сервера. Обёрнутые, в переведённом режиме они перестают совпадать, и кнопка
   «Починить» гаснет на сегментах, которые чинить МОЖНО. Видимого признака
   поломки у этого нет: экран выглядит исправным.

   «е» здесь по той же причине: это ДАННЫЕ — `.replace(/ё/g, "е")` приводит
   «ё» к «е» перед поиском. Обёрнутая, она подставляла в узбекском режиме
   латинское "e", и поиск по слову с «ё» молча переставал работать.
