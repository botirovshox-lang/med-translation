...
IMG_JOIN_GAP = 1.6        # дальше этого не склеиваем никогда, в высотах строки
IMG_JOIN_TIGHT = 0.6      # ближе этого — пробел при любых признаках
IMG_JOIN_EDGE = 2         # столько строк на одном x делают его краем колонки
...
def _written_across(lines: list, x0: float, x1: float,
                    mid: float, h: float) -> bool:
    """По этому месту пишет кто-то ЕЩЁ — значит это пробел, а не межколонник.

    Смотрим строки из ДРУГИХ полос: соседи по своей полосе — это куски той же
    разорванной строки, они стоят по краям зазора и о нём ничего не говорят."""
    need = 0.6 * max(1.0, x1 - x0)
    for l in lines:
        lm = (l["box"][1] + l["box"][3]) / 2
        if abs(lm - mid) <= 0.5 * h:
            continue
        if min(x1, l["box"][2]) - max(x0, l["box"][0]) >= need:
            return True
    return False
...
def _join_pieces(lines: list) -> list:
    ...
    for ln in sorted(lines, key=lambda l: (l["box"][1], l["box"][0])):
        ...
        for cur in out:
            ...
            # Край колонки, попавший в зазор, склейку запрещает. Проверяем оба:
            # колонка бывает выключена и влево, и вправо.
            edge = any(lo - tol <= e <= hi + tol for e in lefts + rights)
            mid = (cur["box"][1] + cur["box"][3]) / 2
            if gap > IMG_JOIN_TIGHT * h and (
                    edge or not _written_across(lines, lo, hi, mid, h)):
                continue
...
def group_blocks(lines: list) -> list:
    ...
    не выровнены и потому в один блок не попадут никогда."""
    lines = _join_pieces(lines)
    blocks: list = []
    for ln in sorted(lines, key=lambda l: (l["box"][1], l["box"][0])):
        x0, y0, x1, y1 = ln["box"]
        h = max(1, y1 - y0)
