...
# ВТОРАЯ форма схлопывания: в скобке не то же слово, а фраза с ним же.
# Боевой #128 второй половиной: ремонт снял «Prevalence (prevalence)», ревизия
# поставила «Prevalence (case prevalence)» — счёт самоповторов не изменился,
# и не видел этого никто. Правило держится на ОРИГИНАЛЕ: в нём скобка есть
# и в ней ДРУГОЙ термин. Замер на боевой книге (2692 сегмента): 4 находки,
# ложных ни одной, прежние не потеряны.
check(len(d("Распространённость (болезненность)", "Prevalence (case prevalence)")) == 1,
      "фраза со своим же головным словом в скобке — находка")
check(d("Распространённость (болезненность)", "Prevalence (morbidity)") == [],
      "верный перевод двух разных терминов чист")
check(d("Микобактерия туберкулёза (M. tuberculosis)",
        "Mycobacterium tuberculosis (M. tuberculosis)") == [],
      "скобка оригинала не из одного слова — правило молчит (латинское название)")
check(d("Туберкулёз (лёгочный туберкулёз)", "Tuberculosis (pulmonary tuberculosis)") == [],
      "автор повторился сам — молчим, отличить наш повтор от его нечем")
check(d("Распространённость болезни", "Prevalence (case prevalence)") == [],
      "в оригинале скобки нет вовсе — сравнивать не с чем")
check(d("аэрогенным (капельным) путём",
        "airborne transmission (airborne droplet and dust-borne transmission)") == [],
      "длинная скобка — пояснение, а не повтор: головное слово входит в него законно")
# Ложные, которых замер на одной книге не поймал (нашёл критик диффа):
# «Термин (уточняющее прилагательное)» — массовая русская конструкция,
# и английский обязан повторить головное слово.
check(d("Туберкулёз (диссеминированный)", "Tuberculosis (disseminated tuberculosis)") == [],
      "в скобке оригинала ОПРЕДЕЛЕНИЕ, а не второй термин — молчим")
check(d("Пневмония (внебольничная)", "Pneumonia (community-acquired pneumonia)") == [],
      "то же на прилагательном другого рода")
check(d("Agreement (Contract)", "Agreement (Service Agreement)") == [],
      "оригинал не кириллицей — правило молчит: замера для этого письма нет")
check(len(d("Распространённость (болезненность)", "case prevalence (case prevalence)")) == 1,
      "одна претензия на один дефект: точное правило и новое не двоят находку")
check(len(d("Распространённость (болезненность)", "Prevalence (case Prevalence)")) == 1,
      "регистр внутри скобки повтору не помеха")
os.environ["SELF_GLOSS_MODE"] = "exact"
check(d("Распространённость (болезненность)", "Prevalence (case prevalence)") == [],
      "правило выключается настройкой, а не правкой кода")
os.environ.pop("SELF_GLOSS_MODE", None)
