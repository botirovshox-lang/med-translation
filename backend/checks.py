...
# ─── Повреждённая формула в САМОМ оригинале ──────────────────────────
# Распознаватель теряет над- и подстрочные знаки. На боевой книге («Пчелиная
# аптека», стр. 161) текстовый слой отдал «и пить его теплым по */ стакана»:
# надстрочная «1» прочитана как «*», подстрочная «3» потеряна СОВСЕМ.
# Восстановить её нечем — её нет в файле, — и выдумывать цифру нельзя:
# выдуманная доза хуже отсутствующей. Всё, что здесь делается, — сказать,
# ГДЕ оригинал повреждён, чтобы человек открыл книгу и вписал число сам
# (правка оригинала прямо в таблице редактора).
#
# Правило СТРУКТУРНОЕ и потому работает на любом языке и в любой области —
# математике, химии, физике, биологии, фармакологии: дробная черта без
# одного из операндов невозможна ни в одном языке. «и/или», «мг/кг», «км/ч»
# не трогаются — там по обе стороны есть, что делить.
_BROKEN_FRACTION_RE = re.compile(r"^[*^]*\d*/\d*[*^]*$")
...
def broken_math(source):
    """Куски оригинала, где формула повреждена распознаванием.

    Ничего не чинит и не подставляет: возвращает сами куски, чтобы их можно
    было назвать человеку и подсветить в строке. Пустой список — молчим,
    как молчат `DOMAIN_RULES` там, где правил нет."""
    s = source or ""
    ...
    for tok in s.split():
        ...
        if _LONE_SCRIPT_RE.match(t):
            out.append(t)
            continue
        if "/" not in t or not _BROKEN_FRACTION_RE.match(t):
            continue
        # Ни цифры, ни знака-обломка — это не дробь, а разделитель: «да / нет»,
        # «ФИО / должность», «п. 5 / 2020 г.». Сервис продаётся не только
        # в медицину, и в договоре такая «находка» звала бы человека править
        # исправный оригинал. Боевой случай («*/») знак-обломок несёт.
        if not any(c.isdigit() or c in "*^" for c in t):
            continue
        left, _, right = t.partition("/")
        # Повреждение — это пустая сторона дроби либо знак-обломок на месте
        # цифры. «1/3» цело и молчит.
        if not left.strip("*^").isdigit() or not right.strip("*^").isdigit():
            out.append(t)
        elif any(c in "*^" for c in t):
            out.append(t)
        if len(out) >= 8:
            break
    return out
