...
# ── Куски одной строки ───────────────────────────────────────────────
# Числа взяты с боевой страницы (image161.png, учебник фтизиатрии): строка
# «рентгенологическим исследовании с» пришла тремя рамками, а до соседней
# колонки зазор ТАКОЙ ЖЕ, как между словами внутри строки. Значит правило
# по ширине зазора обязано ошибиться — и здесь проверяется, что решает
# не ширина, а признаки колонки.
torn = [line(0, 502, 227, 521),        # «рентгенологическим»
        line(247, 502, 399, 520),      # «исследовании»   зазор 20
        line(421, 506, 431, 517)]      # «с»              зазор 22
one = it.group_blocks(torn + [line(21, 381, 416, 405), line(22, 404, 334, 423)])
body = [b for b in one if b["box"][1] >= 500]
check(len(body) == 1 and body[0]["box"] == [0, 502, 431, 521],
      "три куска одной строки — одна строка: %s" % [b["box"] for b in body])

# Та же страница целиком: слева текст, справа подпись к рисунку. Зазор
# до колонки 21 пиксель — между словами было 20 и 22.
page = torn + [line(21, 381, 416, 405), line(22, 404, 334, 423),
               line(453, 501, 862, 524), line(453, 523, 828, 541),
               line(348, 527, 432, 541)]
got = it.group_blocks(page)
check(all(not (b["box"][0] < 440 < b["box"][2]) for b in got),
      "колонка не склеена с соседней при равном зазоре: %s" % [b["box"] for b in got])

# Единственная строка на всей картинке, разорванная надвое: поручиться
# за зазор некому, и широкий мы не склеиваем — но узкий пробел колонкой
# не бывает.
check(len(it.group_blocks([line(10, 10, 100, 28), line(108, 10, 200, 28)])) == 1,
      "узкий пробел склеен и без свидетелей")
check(len(it.group_blocks([line(10, 10, 100, 28), line(135, 10, 220, 28)])) == 2,
      "широкий зазор без свидетелей не склеиваем: это может быть колонка")

# Две подписи в ряд, по одной строке у каждой: краю колонки повториться не на
# чем, и держит только правило «по зазору никто не пишет».
pair = [line(0, 100, 180, 120), line(210, 100, 390, 120)]
check(len(it.group_blocks(pair)) == 2,
      "две однострочные подписи в ряд не склеены: %s" % [b["box"] for b in it.group_blocks(pair)])
