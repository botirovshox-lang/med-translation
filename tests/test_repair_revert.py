...
def fake_bc_worse(s, p, *a, **k):
    """Балла тут МАЛО, чтобы правку отвергли: он вознаграждает кальку, и одно
    его падение больше не отменяет правку, почистившую термины (см.
    test_repair_veto.py). Отвергает её ЖЁСТКАЯ находка — расхождение чисел,
    единиц, отрицания или подмена стороны: она не зависит ни от морфологии,
    ни от буквализма и остаётся вето при любом улучшении терминологии.
    Проверяем же мы здесь другое — что откат возвращает проверку ПРЕЖНЕГО
    текста, а не оставляет свежую от отвергнутого."""
    s["backcheck"] = {"score": 20, "model": "gpt-5.6-luna", "back": "обратный перевод НОВОГО",
                      "target_hash": main._text_hash(s["target"] or ""),
                      "hard": True, "reasons": ["расхождение чисел"]}
    return {"ok": True}
