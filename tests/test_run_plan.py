...
# Имя модели в причинах больше не называется (инвариант 24): разбор
# состава читает человек, а решений по имени модели он не принимает.
check(any("слабее нужной" in r for r in runs), "и причина взять в работу тоже названа")
check(not any("Sol" in r or "Luna" in r or "Terra" in r for r in runs | skips),
      "и ни одна причина не называет модель по имени")
...
check(any("ваша строка" in r["reason"] for r in rp["skips"]),
      "работа человека названа поимённо, а не посчитана как «нечего чинить»")
