...
check(bare.indexOf("приложите его ниже") !== -1,
      "сказано, почему «как оригинал» сейчас не соберётся");
check(bare.indexOf("Приложить исходник") !== -1, "есть кнопка приложить файл");
check(bare.indexOf("Картинки и вид исходника не переносятся") !== -1,
      "простой текст не обещает сохранить оформление");
...
// Карточка не имеет права прятаться при нуле находок: пропавшее с экрана
// выглядит благополучнее, чем есть, а ноль тут бывает и настоящим,
// и следствием того, что разбор просто не запускали.
check(bare.indexOf("Надписи на картинках") !== -1, "карточка про картинки есть и без исходника");
check(bare.indexOf("Сначала приложите исходный .docx") !== -1,
      "без исходника сказано, почему искать надписи не в чем");
const withsrc = render({ file: "book.docx", at: "2026-08-25 10:00", paras: 3346, segments: 2670 });
// Пока сервер не ответил, карточка говорит именно это. Сказать «разбор
// не делался» или «надписей нет» значит выдать незнание за факт о проекте.
check(withsrc.indexOf("Спрашиваем сервер") !== -1,
      "до ответа сервера карточка не утверждает ничего о находках");
...
// Исключений больше нет: выбора модели нет ни у кого (modelsShown
// в ui.jsx). Читать будет модель, назначенная в админке (инвариант 30),
// и это и есть честное умолчание скрытой настройки (инвариант 24).
const withsrcSu = render({ file: "book.docx", at: "2026-08-25 10:00", paras: 3346, segments: 2670 },
                         { owner: true, super: true });
check(withsrcSu.indexOf("Модель чтения") === -1, "и суперпользователю выбор модели тоже не показывается");
check(withsrcSu.indexOf("каталог моделей не загрузился") === -1,
      "…и устройство разбора ему не пересказывается");
check(withsrcSu.indexOf("Найти надписи") !== -1, "…а работа остаётся: кнопка на месте");
...
check(withSrc.indexOf("Даже надписи на картинках переведены") !== -1,
      "формат «как оригинал» подписан тем, что человек получит");
check(withSrc.indexOf("Заменить") !== -1, "исходник можно заменить");
check(withSrc.indexOf("Скачать как оригинал") !== -1,
      "с исходником по умолчанию «как оригинал», и кнопка его называет");
check(bare.indexOf("Скачать DOCX") !== -1 && bare.indexOf("Скачать как оригинал") === -1,
      "без исходника по умолчанию простой текст — «как оригинал» собрать не из чего");

// ── в файл клиента не обещаем внутреннюю кухню ─────────────────────
// Прежние галочки «Результаты QA», «Заметки переводчика», «Ссылки на
// глоссарий» на сервер не уходили и обещали в файле кухню проверок.
for (const [label, txt] of [["без исходника", bare], ["с исходником", withSrc]])
  for (const bad of ["Результаты QA", "Заметки переводчика", "Ссылки на глоссарий"])
    check(txt.indexOf(bad) === -1, label + ": нет галочки «" + bad + "»");
check(bare.indexOf("Оригинал рядом с переводом") !== -1,
      "у простого текста (docx) галочка оригинала есть");
check(withSrc.indexOf("Оригинал рядом с переводом") === -1,
      "у «как оригинал» её нет — этот формат оригинала рядом не кладёт");
