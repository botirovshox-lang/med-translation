...
_RU_ENDINGS = frozenset((
    ...
    # «верхний → верхнее», «средний → среднее», «ранний → раннее». Без него
    # правило снимало законные формы записей, которые в глоссарии есть.
    "ее",
    "ами", "его", "ему", "ими", "ого", "ому", "ыми", "ями", "ией", "иям",
    "иях", "ьев", "ьям", "ьях", "иями", "ьями",
))
_RU_ENDING_MAX = 4
# Короткое слово целиком помещается в начало чужого: «тест» → «тестостерона»,
# «блок» → «блокада», «под» → «подключичная», «ген» → «генерализованный».
# Поэтому послабление «найденное начинается с записи целиком» действует только
# от пяти букв. Короткие записи от этого не теряются: «рак» → «рака»,
# «нос» → «носа», «кома» → «комой» проходят обычной проверкой окончаний
# (пустой остаток в списке есть).
_WHOLE_WORD_MIN = 5
...
def _same_lexeme(term_word: str, found_word: str) -> bool:
    ...
    Два намеренных исключения, оба ради того, чтобы не менять лишнего:
      • найденное НАЧИНАЕТСЯ с записи целиком и запись не короче
        _WHOLE_WORD_MIN («туберкулёз» → «туберкулёзного», «инфильтрат» →
        «инфильтративный») — так работало и так работает. Это НЕ значит,
        что производные прилагательные не судятся вообще: когда запись
        кончается гласной или мягким знаком, послабление не срабатывает,
        и «полость» → «полостной», «бронхит» → «бронхиальный», «хирургия» →
        «хирургический» отвергаются. Правило тут не про части речи,
        а про то, целиком ли запись вошла в найденное слово;
      • слово не из ОДНОЙ ТОЛЬКО кириллицы — МОЛЧИМ. Под это попадает
        не только латиница: «С-реактивный», «Т-лимфоцит», «фиброзно-
        кавернозный» и всё с цифрой или знаком внутри проверку тоже минуют
        (на боевом глоссарии это 4% слов). Список окончаний русский, и для
        такого слова он не значит ничего: тот же закон, что у DOMAIN_RULES
        без правил для пары языков. Вреда мало — такие слова длинные
        и началом друг друга почти не бывают.
...
def _term_match(term: str, text: str) -> str | None:
    ...
    if stem is None:
        return None
    # Перебираем ВСЕ вхождения, а не первое. Стем совпасть мог и с другим
    # словом (см. _same_lexeme), а настоящая форма стоять следующим словом:
    # «Очаговый туберкулез легких. Туберкулемы выявлены…» — по первому хиту
    # запись «Туберкулема» была бы объявлена отсутствующей, и приказ не ушёл
    # бы ни в промпт, ни в проверку соответствия. В учебнике фтизиатрии
    # «туберкулёз» и «туберкулёма» стоят в одном абзаце постоянно, так что
    # правило гасило бы само себя, и молча. Лишние шаги платятся только там,
    # где первое вхождение проверку не прошло.
    tw = (term or "").lower().split()
    for hit in stem.finditer(text):
        found = hit.group(0)
        ...
        if len(tw) != len(fw):
            ...
            continue
        if all(_same_lexeme(a, b) for a, b in zip(tw, fw)):
            return found
    return None
