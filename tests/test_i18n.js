...
const backend = ["backend/main.py", "backend/textcount.py", "backend/store.py"]
  .filter(f => fs.existsSync(f))
  .map(f => fs.readFileSync(f, "utf8")).join("\n")
  /* Питон склеивает соседние литералы сам: длинное сообщение в исходнике
     разрезано переносом строки, а до клиента доезжает целым. Сшиваем так же,
     иначе проверка ругалась бы на верные ключи. */
  .replace(/"[ \t]*\n[ \t]*"/g, "")
  .replace(/'[ \t]*\n[ \t]*'/g, "");
