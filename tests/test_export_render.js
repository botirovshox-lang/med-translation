...
function render(sourceDocx) {
  ...
  const project = {
    ...
    segments: [{ id: 1, source: "жалобы", target: "complaints", status: "confirmed", risk: "low" }],
  };
  if (sourceDocx) project.sourceDocx = sourceDocx;
  const storeStub = {
    activeProject: project, projects: [project], exportHistory: [],
    statusCounts: () => ({ all: 1, new: 0, translated: 0, qa: 0, confirmed: 1, failed: 0, review: 0 }),
    patchProject() {}, go() {},
  };
...
check(withSrc.indexOf("шрифты, картинки, таблицы и разметка на месте") !== -1,
