...
# ── Куски одной строки ───────────────────────────────────────────────
# Числа — с боевой страницы (image161.png, учебник фтизиатрии): слева абзац
# основного текста в четыре строки, справа две подписи к рентгенограммам.
# Третья строка абзаца пришла от детектора ТРЕМЯ рамками, а до соседней
# КОЛОНКИ зазор такой же, как между словами внутри строки (21 против 20 и 22).
# Значит правило по ширине зазора обязано ошибиться, и здесь проверяется,
# что решает не ширина.
PAGE = [line(21, 381, 416, 405), line(22, 404, 334, 423),    # подпись «Рис. 176»
        line(0, 450, 432, 470), line(0, 474, 434, 498),      # абзац, строки 1-2
        line(0, 502, 227, 521), line(247, 502, 399, 520),    # строка 3 — тремя
        line(421, 506, 431, 517),                            #   кусками
        line(348, 527, 432, 541), line(2, 531, 168, 541),    # строка 4, обрезана
        line(453, 501, 862, 524), line(453, 523, 828, 541)]  # подпись «Рис. 177»
rows = it._join_pieces(PAGE)
third = [l for l in rows if 495 <= l["box"][1] <= 505 and l["box"][0] < 100]
check(len(third) == 1 and third[0]["box"] == [0, 502, 431, 521],
      "три куска одной строки — одна строка: %s" % [l["box"] for l in third])
check(all(not (l["box"][0] < 440 < l["box"][2]) for l in rows),
      "колонка не склеена с соседней при равном зазоре: %s" % [l["box"] for l in rows])
blocks = it.group_blocks(rows)
check(any(b["rows"] >= 3 and b["box"][2] <= 440 for b in blocks),
      "абзац собрался в блок: %s" % [(b["box"], b["rows"]) for b in blocks])

# Свидетель зазора обязан накрыть ОБА куска. Строка своей же колонки, которая
# длиннее соседних и заходит за межколонник, ручается за пропасть, о которой
# ничего не знает: так «левый текст + правая подпись» оказывались одним текстом.
ragged = [line(0, 100, 180, 120), line(0, 125, 215, 145), line(0, 150, 195, 170),
          line(205, 100, 400, 120)]
check(len(it._join_pieces(ragged)) == 4,
      "длинная строка своей колонки не ручается за межколонник: %s"
      % [l["box"] for l in it._join_pieces(ragged)])

# И он обязан быть РЯДОМ. Иначе заголовок страницы объявляет пробелом любой
# зазор, какой пересекает, — по всей высоте картинки.
pair = [line(0, 100, 180, 120), line(210, 100, 390, 120)]
check(len(it._join_pieces(pair)) == 2, "две однострочные подписи в ряд не склеены")
check(len(it._join_pieces(pair + [line(0, 10, 390, 30)])) == 3,
      "далёкий заголовок не ручается за чужой зазор: %s"
      % [l["box"] for l in it._join_pieces(pair + [line(0, 10, 390, 30)])])

# Край колонки запрещает склейку при ЛЮБОМ зазоре, включая нахлёст: рамки
# детектора идут с запасом, и рамка соседней колонки, налезшая на пару
# пикселей, склеивалась безусловно — мимо найденного края.
tight = [line(0, 100, 210, 120), line(0, 125, 180, 145),
         line(205, 100, 400, 120), line(205, 125, 400, 145)]
check(all(l["box"][2] <= 400 and (l["box"][0], l["box"][2]) != (0, 400)
          for l in it._join_pieces(tight)) and len(it._join_pieces(tight)) == 4,
      "нахлёст рамок соседних колонок не склеивается: %s"
      % [l["box"] for l in it._join_pieces(tight)])

# Узкий пробел склеиваем и без свидетелей: межколонник такой ширины не бывает.
check(len(it._join_pieces([line(10, 10, 100, 28), line(108, 10, 200, 28)])) == 1,
      "узкий пробел склеен и без свидетелей")
check(len(it._join_pieces([line(10, 10, 100, 28), line(135, 10, 220, 28)])) == 2,
      "широкий зазор без свидетелей не склеиваем: это может быть колонка")

# Два прохода. Прежний разбор терял строку, если увеличенный взгляд видел её
# хуже (боевая image100.png: рамки с уверенностью 0.62 и 0.64 уходили под порог),
# поэтому проходы СКЛАДЫВАЮТСЯ, а одинаковое считается один раз.
same = it._dedupe_boxes([line(10, 10, 200, 30, conf=0.7),
                         line(11, 10, 199, 30, conf=0.9)])
check(len(same) == 1 and same[0]["conf"] == 0.9,
      "одна строка из двух проходов — одна строка, уверенность лучшая")
part = it._dedupe_boxes([line(10, 10, 200, 30), line(12, 12, 90, 28)])
check(len(part) == 1 and part[0]["box"] == [10, 10, 200, 30],
      "кусок внутри целой строки не задваивается")
# Проходы режут строку по-разному, и рамки расходятся внахлёст, а не вложением.
# Оставить обе — два сегмента с общим куском текста: прочитан дважды,
# переведён дважды и дважды написан поверх картинки при выгрузке.
lap = it._dedupe_boxes([line(0, 10, 200, 30), line(210, 10, 300, 30),
                        line(0, 10, 90, 30), line(100, 10, 300, 30)])
check(len(lap) == 1 and lap[0]["box"] == [0, 10, 300, 30],
      "рамки одной строки внахлёст — одна рамка: %s" % [l["box"] for l in lap])
check(len(it._dedupe_boxes([line(10, 10, 100, 30), line(120, 10, 200, 30)])) == 2,
      "разные строки остаются разными")

check(it._detect_scale(877, 542) == 2, "мелкую картинку увеличиваем перед поиском")
check(it._detect_scale(2000, 1500) == 1, "крупной увеличение не нужно")
check(it._detect_scale(1399, 1399) == 1, "потолок по мегапикселям сильнее размера стороны")
...
# Надпись оригинала шла сверху вниз (корешок книги): рамка в одну строку выше
# своей ширины. Перевод мы пишем горизонтально, и раньше он вставал в такую
# рамку столбиком обрубков — на обложке боевой книги рамка 84x377 приняла
# «Руҳоний Александр Лазебный» кеглем 48 в семь строк.
vert, vrep = it.render_target(src, [{"box": [40, 20, 80, 180], "rows": 1,
                                     "text": "Священник Александр Лазебный"}])
check(vert is None and vrep[0]["why"] == "vertical",
      "надпись вдоль корешка не перерисовывается: " + str(vrep[0]))
# Метка схемы чуть выше своей ширины — обычная горизонтальная надпись,
# и порог не имеет права принять её за повёрнутую.
lbl, lrep = it.render_target(src, [{"box": [40, 60, 90, 120], "rows": 1, "text": "Яд"}])
check(lrep[0].get("why") != "vertical", "короткая метка не считается повёрнутой")
# Слова целые: раскладка с резкой «влезает» при любом кегле, и двоичный поиск
# выбирал самый крупный — получалось «Алек/сандр» поперёк картинки.
dr2 = ImageDraw.Draw(Image.new("RGB", (10, 10)))
ffont, frows, _fstep, _fsize = it._fit(dr2, "Руҳоний Александр Лазебный",
                                       it.font_path(), 84.0, 377.0, 377)
check(ffont is not None and all(w in " ".join(frows).split()
                                for w in ("Руҳоний", "Александр", "Лазебный")),
      "кегль подбирается так, чтобы слова не рвались: %s" % (frows,))
webp = io.BytesIO()
...
print("\n── текст строк доезжает до блока ──")
# Серверный разбор текста на этом шаге НЕ имеет: его читает модель уже
# по готовым блокам, и ключ `text` там появиться не должен — по его
# отсутствию решают, о каком блоке спрашивать модель.
plain = it.group_blocks([line(10, 10, 200, 34), line(10, 40, 200, 64)])
check(bool(plain) and "text" not in plain[0],
      "без текста ключа нет вовсе — иначе чтение не спросит ни одного блока")
withtxt = it.group_blocks([
    dict(line(10, 10, 200, 34), text="подпись с пере-"),
    dict(line(10, 40, 200, 64), text="носом")])
check(len(withtxt) == 1 and withtxt[0].get("text") == "подпись с переносом",
      "строки блока склеены, перенос по дефису снят: %r"
      % (withtxt[0].get("text") if withtxt else None))

print("\n── потолок площади ──")
# Картинка приходит из чужого файла, а воркер один: ровный PNG на мегабайты
# разворачивается в гигабайты растра, и память кончится у всего сервиса.
# Отказ обязан быть «не знаю» (None), а не «надписей нет» (пустой список).
_was_cap = it.IMG_MAX_MPX
it.IMG_MAX_MPX = 0.0001                     # заведомо меньше тестовой картинки
try:
    check(it.detect_lines(src) is None,
          "картинка больше потолка площади — «не знаю», а не пустой список")
    check(it.crop(src, BOX) is None, "кроп непомерной картинки — отказ")
    check(it.preview(src) is None, "обзорный кадр непомерной картинки — отказ")
    blob, rep = it.render_target(src, [{"box": list(BOX), "text": "x"}])
    check(blob is None and bool(rep) and not rep[0]["ok"],
          "перерисовка непомерной картинки — отказ с отчётом по каждому блоку")
finally:
    it.IMG_MAX_MPX = _was_cap
check(it.detect_lines(b"not an image") is None,
      "потолок снят — мусор вместо картинки по-прежнему «не знаю»")

print("\n── ответ движка ──")
# Движок отвечает двумя видами: полный проход отдаёт (рамка, текст,
# уверенность), поиск БЕЗ чтения — голую рамку. Уверенности там нет вовсе,
# и подставить вместо неё число нельзя: по нему склеивают строки и его же
# показывают человеку в списке отсеянного.
_pts = [[10, 10], [90, 10], [90, 30], [10, 30]]
check(it._unpack([_pts, "text", 0.88]) == (_pts, 0.88), "полный ответ разобран")
check(it._unpack(_pts) == (_pts, None), "ответ без чтения — рамка и «не знаю»")
check(it._unpack("мусор") == (None, None), "непонятный ответ — не срыв, а отказ")
check(it._conf_best(0.7, None) == 0.7, "«не знаю» не перебивает известное число")
check(it._conf_worst(0.7, None) is None, "«не знаю» хуже любого числа")
check(it._conf_all([line(0, 0, 9, 9, conf=None)]) is None,
      "блок из строк без уверенности — «не знаю», а не выдуманное число")
