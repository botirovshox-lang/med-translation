...
(function () {
  ...
  /* Распознавалка: высота строки 48 пикселей — это вход модели, а не вкус.
     Ширина у модели динамическая, и REC_W_MAX — это не «вход», а решение,
     с какого места строку РЕЗАТЬ. Число замерено в настоящем браузере:
     при 640 подпись «Диссеминированный туберкулёз лёгких» (467×23, то есть
     975 пикселей после подгонки по высоте) резалась надвое, и шов съедал
     пробел — «Обзорнаярентгенограмма», — а в соседней строке добавлял букву.
     Сжатие же до 66% она пережила без единой ошибки. Значит сжатие дешевле
     шва, и резать надо только там, где сжимать уже нельзя. */
  var REC_H = 48, REC_W_MAX = 1600;
  ...
  function loadOrt() {
    ...
    ortP = new Promise(function (done, fail) {
      ...
      s.onload = function () {
        ...
        /* АБСОЛЮТНЫЙ адрес, а не «vendor/ocr/». Исполнитель подтягивает
           свой `.mjs` динамическим import'ом, а туда относительный путь
           уходит как ИМЯ МОДУЛЯ — браузер отвечает «не могу разрешить
           спецификатор» и чтение не поднимается вовсе. Поймано только
           настоящим браузером: в node этого пути нет. */
        window.ort.env.wasm.wasmPaths = new URL(DIR, document.baseURI).href;
        done(window.ort);
      };
