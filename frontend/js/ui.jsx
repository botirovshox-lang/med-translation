...
/* ---------- Называются ли модели на этом экране ----------
   ...
   Читается из хранилища (`store.can.super`), а не из кэша в localStorage:
   умолчание `{owner:false, super:false}` до первого ответа сервера прячет
   имя, то есть ошибается в безопасную сторону. Упрощённый режим организации
   (инвариант 22) прячет модели и суперпользователю: там их назначает
   организация, и называть назначенное нельзя никому. */
function modelsShown(store) {
  return !!(store && store.can && store.can.super) && !costHidden();
}
