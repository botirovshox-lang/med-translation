...
Что сторожится и почему именно это:
  ...
  1. Смета продолжает СЧИТАТЬСЯ и уходить на сервер У ТЕХ, КОМУ ЕЁ ПОКАЗЫВАЮТ.
     На ней стоят отказ по смете на старте прогона и калибровка `estRatio`.
     А цены в каталоге режет тот же денежный рубеж, что и всё остальное
     (инвариант 22а): смету видит тот, кто платит, и считать её по ценам,
     которых человеку не показывают, незачем. Отказ 402 от этого не пропал —
     он «защита от случайности», а жёсткий рубеж стоит МЕЖДУ ПОРЦИЯМИ
     и меряет ФАКТ расхода, а не обещание браузера.
...
# Смотрим СУПЕРОМ: деньги видит только администратор сервиса (инвариант 22а),
# и «режим ещё не включён» проверяется на нём — иначе проверка мерила бы
# не режим организации, а роль.
cat = c.get("/api/models", headers=H(S)).json()
check(cat["hideCost"] is False, "обычная организация: администратору суммы показываются")
check(all("in" in m for m in cat["models"]), "цены в каталоге есть")
check(c.get("/api/models", headers=H(T)).json()["hideCost"] is True,
      "а владельцу организации — нет, и до включения режима тоже")
...
# Прежде цены оставались всем «ради сметы». Теперь их режет тот же денежный
# рубеж: смету видит тот, кто платит, а в упрощённом режиме не видит никто —
# и считать её не по чему. Отказ 402 на старте от этого не пропал: он лишь
# «защита от случайности», а жёсткий рубеж стоит МЕЖДУ ПОРЦИЯМИ (`_job_run`)
# и меряет ФАКТ, а не обещание браузера.
check(all("in" not in m and "out" not in m for m in cat["models"]),
      "цен в каталоге нет: в упрощённом режиме сумм не видит никто")
